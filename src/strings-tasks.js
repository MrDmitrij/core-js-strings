...
function sumOfCodes(str) {
  return str
    ? Array.from(str).reduce((sum, current) => sum + current.charCodeAt(), 0)
    : 0;
}
  ...
  return str
    ...
    .reduce(
      (result, char, index, arr) =>
        !!(result && char === arr[arr.length - index - 1]),
      true
    );
}
...
function findLongestWord(sentence) {
  return sentence
    ...
    .find((item, index, arr) => item.length === arr.at(-1).length);
}
...
function invertCase(str) {
  ...
  return str
    .split('')
    .map((value) =>
      char.includes(value) ? value.toUpperCase() : value.toLowerCase()
    )
    .join('');
}
